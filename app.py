...
def show_kategorial():
    ...
    if st.session_state.uploaded_files:
        ...
        try:
            ...
            st.error(f"Terjadi kesalahan saat membaca sheet 'kategorial': {e}")
    else:
        ...
        try:
            ...
            if st.button('Lakukan Perhitungan K-Medoids'):
                try:
                    ...
                    if len(set(labels)) > 1:  # Ensure more than 1 cluster for silhouette score calculation
            ...
            if st.button('Cari Jumlah Cluster Terbaik'):
                silhouette_scores = []
                K = range(2, min(11, len(df_perhitungan)))  # Limit K to not exceed the number of samples
                for k in K:
                    ...
                    if len(set(labels)) > 1:  # Ensure more than 1 cluster
                        ...
                        silhouette_scores.append(-1)  # Negative value for invalid cluster count
            ...
            st.error(f"Terjadi kesalahan saat membaca sheet 'number': {e}")
    else:
        st.write("Silakan unggah file Excel terlebih dahulu di menu Data.")
